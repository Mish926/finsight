...
- **Segment-vs-firm-wide disambiguation**: chunks that look like a business-segment breakdown (named divisions repeated across years) are automatically de-prioritized for questions that ask about the company as a whole, not a specific segment.
...
3. Click **API Keys** then **Create API Key**, and copy it
...
Planner and Critic just need to decompose questions and judge evidence sufficiency, so the fast, cheap 8B model handles that well. The Synthesizer does the actual hard reasoning (reading multi-year tables correctly, not confusing a segment total for a firm-wide one), so it runs on the larger 70B model, with automatic fallback to the 8B model if it's ever unavailable.
